...
- [ ] **Phase 3 — PDF-upload autofill (remaining half — current phase).** Upload an existing resume PDF and have the profile form pre-filled via AI-structured extraction — see plan doc's "Profile PDF upload & autofill" section; this is the one deliberate AI exception in an otherwise AI-free phase.
  - [ ] Backend `POST /api/profile/parse-resume` + `ChatAnthropic.withStructuredOutput`
  - [ ] Frontend `pdfjs-dist` text extraction + form pre-fill
  - [ ] Test end-to-end
...
- [ ] **Phase 7 — Cross-application insights (Patterns page)** (aggregate `FeedbackReport`s in code, one `ChatAnthropic.withStructuredOutput` call turns that into recurring patterns + technologies to study + a learning roadmap — replaces today's mock data on `/patterns`; no LangGraph needed, this isn't an agentic/multi-step workflow, see plan doc's "Cross-application insights (Patterns page)" section)
- [ ] **Phase 8 — Embeddings + RAG grounding** (OpenAI embeddings + MongoDB Atlas Vector Search)
- [ ] **Phase 9 — Stretch/polish** (status UX, resume version history, cover letters, thumbnails, rate limiting, tests)
